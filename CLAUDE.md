...
app-audio-recorder record --output ~/Desktop/call      # 指定基础路径，派生三文件
...
- **录麦克风（默认）**：产出三个文件——`<base>-app.wav`（app 音频）、`<base>-mic.wav`（麦克风）、`<base>-merged.wav`（两路离线混音）。
- **`--no-mic`**：只产出 `<base>.wav` 单文件。
- 其中 `<base>` 由 `--output` 指定（去掉 `.wav` 扩展名），否则为当前目录 `<app名>-<时间戳>`。
...
| [RecordCommand.swift](Sources/AppAudioRecorder/RecordCommand.swift) | `record`：参数解析、派生输出路径、启动引擎、Ctrl-C/定时停止、触发合并 |
...
| [AudioMerger.swift](Sources/AppAudioRecorder/AudioMerger.swift) | 停止后把 app/mic 两路 WAV 离线混音成 merged 文件 |
...
5. 停止后，若录了麦克风，[AudioMerger](Sources/AppAudioRecorder/AudioMerger.swift) 把两路读回、统一到浮点格式、逐样本相加并硬限幅，写出 merged 文件。

## 关键约束（改代码前务必注意）

- **线程模型**：[AudioCaptureEngine](Sources/AppAudioRecorder/AudioCaptureEngine.swift) 的可变状态（`appWriter`/`micWriter`/`writeError`）与 [WAVWriter](Sources/AppAudioRecorder/WAVWriter.swift) 的全部状态都只在串行队列 `sampleQueue` 上访问——ScreenCaptureKit 的 delegate 回调固定在该队列触发。app 与麦克风两路共用同一 `sampleQueue`，按 `SCStreamOutputType` 分发到各自的 `WAVWriter`。引擎的 `@unchecked Sendable` 是为桥接这套回调式 API 的**有依据**标注（`WAVWriter` 因此可保持 `nonisolated` 非 Sendable、无需加锁）。**新增可变状态必须走同一队列**，否则破坏该前提。
- **文件关闭时机**：`stop()` 在 `sampleQueue` 上同步调用两路 `writer.finish()`（置空文件句柄）以写回 WAV 头长度；不要改成异步或提前释放。
- **合并时机与线程**：合并**必须在 `stop()` 之后**（两路文件头已写回、可安全读回）；[AudioMerger](Sources/AppAudioRecorder/AudioMerger.swift) 是 CPU 密集的纯计算，由 `RecordCommand` 用 `@concurrent` 放到并发线程池执行，**不要占用协作线程池或采样队列**。
...
- **输出格式**：落盘固定 16-bit PCM WAV；采样率/声道由 `--sample-rate`/`--channels` 决定（默认 48kHz 立体声），`processingFormat` 与 ScreenCaptureKit 输入对齐。合并输出以 app 音轨的采样率/声道为目标格式；两路时长不一致时以较长者为准，较短一路缺失部分按静音补齐。

## 已知限制与扩展点

- **麦克风混录**：默认同时录麦克风并产出 app/mic/merged 三文件（`--no-mic` 回到 app 单文件）。合并为停止后离线混音，逐样本相加并硬限幅防削波，不做增益归一化或降噪。
