...
enum ContentResolver {
    ...
    static func runningApps() async throws -> [SCRunningApplication] {
    ...
    static func findApp(matching query: String) async throws -> SCRunningApplication {
        let apps = try await runningApps()
        guard !apps.isEmpty else { throw RecorderError.noAppsAvailable }
        ...
        throw RecorderError.appNotFound(query: query)
    }

    /// 为指定 app 构建「只包含该 app 音频」的过滤器。
    static func filter(for app: SCRunningApplication) async throws -> SCContentFilter {
        let content = try await shareableContent()
